...
int		get_next_line_sub(int fd, char **line)
{
	...
	if (exist_newline_in_remain_string(remain_string, fd, line) == 1)
		return (1);
	ft_memset2(buffer, 0, (BUFFER_SIZE + 1)); //read로 담기 이전에 buffer를 깨끗이 청소해줌
	...
	{
		// *line = ft_strdup2(""); 넣어주면 좋긴한데, 줄 수 제한때문에 빼도록한다.
		return (-1);
	}
...
// change_newline_to_zero()함수를 통해 개행이 있을 때, return_line과 static_line으로 분절시키고, 개행이 없으면 staitc variable에 계속 문장을 붙여줌(ft_strjoin2)
